class Solution {
    public int characterReplacement(String s, int k) {
        ...
        while(rptr<s.length()){
            ...
            // increament the char count of the current character
            charCount[s.charAt(rptr) - 'A']++;

            // calcluate the max-frequency for the characters encountered yet
            maxFrequency = Math.max(maxFrequency, charCount[s.charAt(rptr) - 'A']);

            // number of change which we can do is length - maxFrequency and it should be <=k
            int changes = (rptr-lptr+1) - maxFrequency;

            while(changes>k){
                charCount[s.charAt(lptr) - 'A']--;
                lptr++;
                changes = (rptr-lptr+1) - maxFrequency;
            }
            if(changes<=k){
